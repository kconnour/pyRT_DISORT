...
class _EmpiricalPhaseFunction(np.ndarray):
    """Designate an array as representing an empirical phase function.

    Parameters
    ----------
    array
        Any valid phase function.
    ...
    def __raise_value_error_if_array_is_not_positive_finite(obj: np.ndarray):
        if ((obj < 0) | ~np.isfinite(obj)).any():
            message = 'Some values in the empirical phase function are ' \
    ...
    def __raise_value_error_if_array_is_not_1d(obj: np.ndarray):
        if obj.ndim != 1:
            message = 'The empirical phase function should be 1D.'
            raise ValueError(message)


class _PhaseFunctionAngles(np.ndarray):
    """Designate an array as representing the angles where the empirical phase
    function is defined.

    Parameters
    ----------
    array
        Any angles where the phase function is defined.
    ...
    def __array_finalize__(self, obj: np.ndarray):
        self.__raise_value_error_if_array_is_within_pi_radians(obj)
        self.__raise_value_error_if_array_is_not_1d(obj)
        self.__raise_value_error_if_array_is_not_monotonically_inc(obj)

    @staticmethod
    def __raise_value_error_if_array_is_within_pi_radians(obj: np.ndarray):
